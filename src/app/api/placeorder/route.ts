...
export async function POST(req: NextRequest) {
  try {
    ...
    const { token, quantity, price, buy, activeAccount } = body;
    ...
    const tx = prepareContractCall({
      contract,
      method: "function placeOrder(address token, uint quantity, uint price, bool buy) public payable",
      params: [token, quantity, price, buy],
      value: toWei(`${price}`)
    });
