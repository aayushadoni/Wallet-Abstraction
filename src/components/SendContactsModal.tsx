...
const SendContactsModal: React.FC<SendModalProps> = ({ showSendModal, toggleSendModal, sendData }) => {
  ...
  const [selectedToChain, setSelectedToChain] = useState(supportedChains[0]);
  const [selectedFromChain, setSelectedFromChain] = useState(supportedChains[0]);
  const [selectedToken, setSelectedToken] = useState(supportedChains[0]);
          ...
          <div className="bg-gray-900 rounded-lg shadow-lg p-4 max-w-md w-full mx-4">
            ...
            <form onSubmit={handleSendSubmit} className="space-y-4 justify-center">
              ...
              <div className="flex items-center">
                <input onChange={() => { toggleCrossChain() }} type="checkbox" className="form-checkbox h-4 w-4 text-blue-600" />
